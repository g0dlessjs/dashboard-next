            ...
            <div className="flex flex-col lg:flex-row gap-8 w-full max-w-6xl">
                ...
                <div className="flex flex-col w-full lg:w-2/5 gap-6">
                    <div
                        ...
                        style={{ background: secondType ? `linear-gradient(135deg, ${getTypeColor(mainType)}, ${getTypeColor(secondType)})` : getTypeColor(mainType) }}
                    >
                        <div className="bg-white rounded-[2.9rem] p-8 flex flex-col items-center relative overflow-hidden group h-full">
                            ...
                            <div
                                ...
                                style={{ backgroundImage: `radial-gradient(circle at 2px 2px, ${getTypeColor(mainType)} 1px, transparent 0)`, backgroundSize: '24px 24px' }}
                            ></div>
                            ...
                            <div className="mt-10 flex gap-12 z-10 w-full border-t border-slate-50 pt-8 justify-center">
                                <div className="text-center">
                                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Peso</p>
                                    <p className="text-2xl font-black text-slate-700">{pokemon.weight / 10}<span className="text-xs ml-1 text-slate-400">kg</span></p>
                                </div>
                                <div className="w-px h-12 bg-slate-100"></div>
                                <div className="text-center">
                                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Altura</p>
                                    <p className="text-2xl font-black text-slate-700">{pokemon.height / 10}<span className="text-xs ml-1 text-slate-400">m</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-xl shadow-slate-200/50">
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-6 text-center">Formas de Combate</p>
                        <div className="grid grid-cols-2 gap-6">
                            {[
                                { label: 'Frente', norm: pokemon.sprites.front_default, shin: pokemon.sprites.front_shiny },
                                { label: 'Espalda', norm: pokemon.sprites.back_default, shin: pokemon.sprites.back_shiny }
                            ].map((item, idx) => (
                                <div key={idx} className="group/variant relative bg-slate-50 rounded-[2rem] p-6 flex flex-col items-center transition-all duration-500 hover:bg-white hover:shadow-2xl hover:shadow-slate-200 border border-transparent hover:border-slate-100 overflow-hidden cursor-help">
                                    ...
                                    <span className="mt-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.label}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[3rem] p-12 shadow-2xl shadow-slate-200/60 border border-slate-100 relative overflow-hidden">
                        <div className="flex items-center justify-between mb-12">
                            <h2 className="text-3xl font-black text-slate-800 flex items-center gap-4">
                                <span className="w-3 h-10 rounded-full" style={{ backgroundColor: getTypeColor(mainType) }}></span>
                                Estadísticas
                            </h2>
                            <span className="text-[11px] font-black text-white px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg" style={{ backgroundColor: getTypeColor(mainType) }}>Base 2024</span>
                        </div>
                                ...
                                <div key={stat.stat.name} className="relative">
                                    <div className="flex justify-between items-end px-1 mb-3">
                                        <div className="flex items-center gap-3">
                                            <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: getStatColor(stat.stat.name) }}></div>
                                        ...
                                        <div className="flex items-baseline gap-1.5 bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-300">/ 255</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="h-5 w-full bg-slate-50 rounded-2xl border border-slate-100 relative p-1 overflow-hidden">
                                        ...
                                        <div className="absolute inset-0 flex justify-between px-4 pointer-events-none opacity-20">
                                            {[...Array(4)].map((_, i) => <div key={i} className="w-px h-full bg-slate-200"></div>)}
                                        </div>
                            ...
                            {pokemon.abilities.map(ability => (
                                ...
                                >
                                    <div className="flex justify-between items-center mb-3">
                                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full ${ability.is_hidden ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-700 text-slate-400'}`}>
...
function getTypeColor(type: string): string {
    const colors: Record<string, string> = {
        fire: '#FF421C', water: '#2F9AFF', grass: '#63BB5B', electric: '#FFD32E',
        ice: '#51C4E7', fighting: '#C56E5C', poison: '#924593', ground: '#D3A328',
        flying: '#A193FF', psychic: '#FF4692', bug: '#ABBC1C', rock: '#A58E49',
        ghost: '#6060B2', dragon: '#6E5CE6', dark: '#595761', steel: '#8A8A9D',
        fairy: '#fb89fb', normal: '#919191'
    };
