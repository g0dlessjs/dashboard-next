...
// En build time se construye
export async function generateStaticParams() {
  const res = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=151`);

  const data: PokemonsResponse = await res.json();
  const static151Pookemons = data.results.map((pokemon) => pokemon.name);
  return static151Pookemons.map((name) => ({ name }));
}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pokemon.abilities.map((ability) => (
                <div
                  ...
                  className={`relative p-8 rounded-[2rem] border transition-all duration-500 hover:-translate-y-1 ${
                    ability.is_hidden
